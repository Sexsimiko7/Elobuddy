...
namespace SexsiPrediction.Skillshots
{
    ...
    internal class PredictionImpl : IPrediction
    {
        ...
        public PredictionImpl()
        {
            ...
            var objAiHeroes = heroes as AIHeroClient[] ?? heroes.ToArray();
        ...
        private void OnNewPath(Obj_AI_Base sender, GameObjectNewPathEventArgs e)
        {
                ...
                if (mec.Radius <= radius + unit.BoundingRadius - 8)
                {
                    return new PredictionOutput
                    {
                        CastPosition = new Vector3(
                            mec.Center.X,
                            NavMesh.GetHeightForPosition(mec.Center.X, mec.Center.Y),
                            mec.Center.Y),
        ...
        private PredictionOutput GetCastPosition(Obj_AI_Base unit, float delay, float radius, float range, float speed, Vector3 from, bool collision, SkillType type)
        {
            ...
            // todo collision
            if (collision && Collision.Collision.CheckCollision(unit, castPosition, delay, radius, range, speed, from))
            {
                hitchance = -1;
            }
